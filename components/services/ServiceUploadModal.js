...
export default function ServiceUploadModal({
  ...
  return ReactDOM.createPortal(
    <AnimatePresence>
      {open && (
        <motion.div
          className="fixed inset-0 z-[9999] flex justify-center items-center bg-gradient-to-br from-black/50 via-cyan-900/60 to-black/50 backdrop-blur-[4px]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          {/* مودال نفسه */}
          <motion.div
            className="relative bg-gradient-to-br from-white via-cyan-50 to-cyan-100 rounded-2xl shadow-2xl px-6 py-8 w-full max-w-lg border border-cyan-300 flex flex-col items-center animate-none"
          ...
          >
            {/* زر إغلاق */}
            <button
              className="absolute top-3 right-3 bg-white/80 hover:bg-red-500 text-gray-400 hover:text-white rounded-full w-8 h-8 flex items-center justify-center text-xl shadow-lg transition-all duration-200 z-10"
              onClick={onClose}
