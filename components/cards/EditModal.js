...
const translations = {
  ar: {
    modalTitle: "تحديث البيانات",
    modalSubtitle: "يرجى رفع صور الإقامة (أمامية/خلفية) وجواز السفر",
  ...
  en: {
    ...
    modalSubtitle: "Please upload Residence ID (Front/Back) and Passport images",
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" dir={locale === "ar" ? "rtl" : "ltr"} lang={locale}>
      <div
        className="
          ...
          border border-emerald-200 relative animate-fade-in
          max-h-[95vh] overflow-y-auto
        "
        style={{ boxShadow: "0 4px 32px 0 rgba(0,0,0,0.10)" }}
      >
        {/* زر إغلاق كبير وبارز على الموبايل */}
        <button
          onClick={onClose}
          className="
        ...
        <div className="flex flex-col items-center mb-2 mt-4 sm:mt-0">
          ...
          <h2 className="text-lg sm:text-xl font-extrabold text-emerald-800 text-center mb-1">{t.modalTitle}</h2>
